...
def exec(case_id, path, dcm_path, model_path, bmp_path, cut_bmp_path, nii_path, nii_name, nii_obj, obj_path, obj_name):
    ...
    min_list.sort(key=lambda x: x[:-4])
    IMG_SIZE = 300
    for x in min_list:
        ...
        quality = 70
        cv2.imwrite(img_name, new_array, [int(cv2.IMWRITE_JPEG_QUALITY), quality])
    print('bbbbbbbbbb')
...
def execute():
    ...
    for path in paths:
        ...
        model_path = os.path.join(filepath + '/domo/app/static/ckp/' + path + '.pth')
...
def smoothDegreeExec():
          ...
          200:
            ...
            schema:
              ...
              properties:
                code:
                  type: integer
                  description: 0: success 10001: 图片选取有误
                data:
                  type: string
    ...
    # label_file = cv2.imdecode(label, 0)
